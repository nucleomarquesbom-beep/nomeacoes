...
// Each PDF table has a header. We learn the real X coordinates from that header,
// instead of assuming fixed coordinates for every FPF document.
...
async function loadFirst(key,urls){for(const u of urls){const img=await tryImage(u+'?v=2');if(img){state.assets.set(key,img);return img;}}return null;}
async function loadIdentity(){
  await loadFirst('logo',['/fotografias/logo.png','/fotografias/logo.jpeg','/fotografias/logo.jpg']);
...
function teamVariants(team){const a=new Set([team.trim(),team.replace(/\bSAD\b/ig,'').replace(/\bSDUQ\b/ig,'').trim(),team.replace(/[,.]/g,'').trim()]);return [...a].filter(Boolean);}
async function shieldImage(team){const key='s:'+compact(team);if(state.assets.has(key))return state.assets.get(key);for(const v of teamVariants(team)){const f=safeFile(v);for(const ext of ['png','jpg','jpeg','webp']){const img=await tryImage(`/escudos/${f}.${ext}?v=2`);if(img){state.assets.set(key,img);return img;}}}return null;}

function drawContain(ctx,img,x,y,w,h){const iw=img.naturalWidth||img.width,ih=img.naturalHeight||img.height,s=Math.min(w/iw,h/ih),dw=iw*s,dh=ih*s;ctx.drawImage(img,x+(w-dw)/2,y+(h-dh)/2,dw,dh);}
function drawCover(ctx,img,x,y,w,h,r=0){const iw=img.naturalWidth||img.width,ih=img.naturalHeight||img.height,s=Math.max(w/iw,h/ih),dw=iw*s,dh=ih*s;ctx.save();if(r){ctx.beginPath();ctx.roundRect(x,y,w,h,r);ctx.clip();}ctx.drawImage(img,x+(w-dw)/2,y+(h-dh)/2,dw,dh);ctx.restore();}
function wrap(ctx,text,x,y,maxWidth,lineHeight,maxLines=2){const words=text.split(/\s+/),lines=[];let line='';for(const w of words){const t=line?line+' '+w:w;if(ctx.measureText(t).width<=maxWidth)line=t;else{if(line)lines.push(line);line=w;}}if(line)lines.push(line);lines.slice(0,maxLines).forEach((l,i)=>ctx.fillText(l,x,y+i*lineHeight));}
...
async function generateAll(){if(!state.games.length)return;const ok=await checkAssets(state.games);if(!ok)return;setStatus('A gerar os JPG...');const zip=new JSZip();for(const g of state.games){const canvas=await render(g);const blob=await new Promise(r=>canvas.toBlob(r,'image/jpeg',.96));zip.file(safeFile(`${g.home} - ${g.away}.jpg`).slice(0,150),blob);}const blob=await zip.generateAsync({type:'blob'});download(blob,'Nomeacoes_Marques_Bom.zip');setStatus(`Concluído: ${state.games.length} JPG(s) gerados.`);}
async function generateManual(){const home=$('mHome').value.trim(),away=$('mAway').value.trim(),competition=$('mCompetition').value.trim();if(!home||!away||!competition)return setError('Preenche competição e as duas equipas.');const officials=[...document.querySelectorAll('.manualOfficial')].map(r=>({name:r.querySelector('.mName').value.trim(),role:r.querySelector('.mRole').value.trim()||'Árbitro'})).filter(x=>x.name);if(!officials.length)return setError('Adiciona pelo menos um oficial.');const g={home,away,competition,officials};if($('mDate').value.trim())g.date=$('mDate').value.trim();const ok=await checkAssets([g]);if(!ok)return;const canvas=await render(g);const blob=await new Promise(r=>canvas.toBlob(r,'image/jpeg',.96));download(blob,safeFile(`${home} - ${away}.jpg`));setStatus('Nomeação manual gerada.');}
